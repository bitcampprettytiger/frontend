...
export const useSearch = (initialSearchInput, location) => {
    ...
    const handleSearchClick = async () => {
        ...
        try {
            ...
            if (geoLocation && geoLocation.lat && geoLocation.lng) {
                ...
                const response = await axios.get(`http://192.168.0.240  /vendor/info?search=${searchInput}`, {
                    headers: getHeaders()
                });
    ...
    useEffect(() => {
        if (searchInput) {
            axios.get(`http://192.168.0.240  /vendor/category?address=${searchInput}&menuName=${searchInput}&vendorName=${searchInput}`, {
                headers: getHeaders()
            })
                .then(response => {
