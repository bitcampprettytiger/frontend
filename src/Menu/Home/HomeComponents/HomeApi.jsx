...
export const fetchShopsInArea = async (areaName) => {
  try {
    ...
    const response = await axios.post(
      `${API_BASE_URL}/vendor/search10vendor`,
      { name: areaName },
      {
        headers: getHeaders(),
      }
    ...
    console.error(
      `${areaName} 지역의 매장 정보를 가져오는 중 오류 발생:`,
      error
    );
    return null;
  }
...
export const fetchReviewsByMemberId = async () => {
  ...
  try {
    const response = await axios.get(`${API_BASE_URL}/myPage/myReviews`, {
      headers: headers,
    });
...
// VendorId를 통해 리뷰 가져오기
export const fetchReviewsByVendorId = async (vendorId) => {
  const headers = getHeaders();
  if (!vendorId) {
    throw new Error('vendorId가 필요합니다.');
  }

  try {
    const url = `${API_BASE_URL}/myPage/myReviews?vendorId=${vendorId}`;
    ...
    console.error('특정 vendorId에 대한 리뷰 정보 가져오기 오류:', error);
    throw error;
  }
};
...
export const fetchTop5Vendors = async () => {
  try {
    ...
    console.error(
      'An error occurred while fetching the top 5 vendors: ',
      error
    );
    return null;
  }
...
export const fetchMyFavoriteVendors = async () => {
  try {
    const response = await axios.get(
      `${API_BASE_URL}/myPage/myFavoriteVendors`,
      {
        headers: getHeaders(),
      }
    );
...
export const updatePaymentInfo = (paymentInfo) => {
  return axios.put(
    `${API_BASE_URL}/api/payments/${paymentInfo.userId}`,
    paymentInfo,
    {
      headers: getHeaders(),
    }
...
export const fetchTop10RecommendedMenus = async () => {
  try {
    const response = await axios.get(
      `${API_BASE_URL}/menu/recommendedMenus10`,
      {
        headers: getHeaders(),
      }
    );
...
export const getVendorByCategory = (searchInput) => {
  return axios.get(
    `${API_BASE_URL}/vendor/category?address=${searchInput}&menuName=${searchInput}&vendorName=${searchInput}`,
    {
      headers: getHeaders(),
    }
  );
...
export const updateMemberInfo = async (updatedInfo) => {
  ...
  try {
    const response = await axios.put(
      `${API_BASE_URL}/myPage/member/update`,
      updatedInfo,
      {
        headers: getHeaders(),
      }
