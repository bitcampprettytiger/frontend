...
export const fetchPopularPlaces = (address, latitude, longitude) => {
    ...
    return axios.post('http://192.168.0.240:1004/vendor/search', {
        address: address,
        latitude: latitude,
        hardness: longitude
    }, {
...
export const fetchMostFavoritedVendors = () => {
    return axios.get('http://192.168.0.240:1004/api/favoritePick/top8Favorites', {
        headers: getHeaders()
    });
...
export const fetchTop5ReviewVendors = async () => {
    try {
        ...
        const response = await axios.get('http://192.168.0.240:1004/vendor/review/averageReviewScore', {
            headers: getHeaders()
        });
...
export const fetchReviewsByVendorId = (vendorId) => {
    return axios.get(`http://192.168.0.240:1004/vendor/review-list/${vendorId}`);
};
...
export const updateReview = (reviewDto, uploadFiles, changeFileList, originFileList) => {
    ...
    return axios.put('http://192.168.0.240:1004/review', formData, {
        headers: getHeaders(),
    });
};
//리뷰삭제
export const deleteReview = (reviewDto) => {
    return axios.delete('http://192.168.0.240:1004/review', { data: reviewDto });
};
//즐겨찾기가 되어 있는 가게리스트
export const fetchFavoriteShopsByUserId = (memberId, token) => {
    ...
    return axios.get(`http://192.168.0.240:1004/api/favorite/${memberId}`, config);
};
// // 즐겨찾기에서 가게를 삭제
// export const deleteFavoriteShop = (memberId, vendorId) => {
//     return axios.delete(`http:/192.168.0.240:1004/api/favoritePick/${memberId}/remove/${vendorId}`, {
//         headers: getHeaders()
//     });
// };
...
export const getMyCart = (memberId, token) => {
    ...
    return axios.get(`http://192.168.0.240:1004/cart/info/${memberId}`, config);
};
...
export const deleteCartItem = (cartItemDTO, token) => {
    ...
    return axios.delete(`http://192.168.0.240:1004/cart/deletecartitem`, { data: cartItemDTO, headers: config.headers });
};
//장바구니비우기
export const deleteCart = (cartItemDTO, token) => {
    ...
    return axios.delete(`http://192.168.0.240:1004/cart/info`, { data: cartItemDTO, headers: config.headers });
};
//조회수가높은10개
export const fetchTop10RecommendedMenus = async () => {
    try {
        const response = await axios.get('http://192.168.0.240:1004/menu/recommendedMenus10', {
            headers: getHeaders()
        });
        if (response.status === 200) {
...
export const fetchMyInfo = async () => {
    try {
        const response = await axios.get('http://192.168.0.240:1004/myInfo', {
            headers: getHeaders()
        });
        if (response.status === 200) {
            return response.data;
...
export const fetchMyFavoriteVendors = async () => {
    try {
        const response = await axios.get('http://192.168.0.240:1004/myPage/myFavoriteVendors', {
            headers: getHeaders()
        });
        if (response.status === 200) {
            return response.data;
...
export const fetchOrderDetail = async (MEMBER_ID) => {
    try {
        const response = await fetch(`http://192.168.0.240:1004/myPage/myOrders`, {
            headers: getHeaders()
        });
...
export const fetchPaymentList = async (token) => {
    try {
        const response = await fetch('http://192.168.0.240:1004/myPage/myPaymentList', {
            headers: getHeaders(),
        });
...
export const createReview = async (reviewDto, file, token) => {
    ...
    const url = 'http://192.168.0.240:1004  /reviews/review';
    const formData = new FormData();
