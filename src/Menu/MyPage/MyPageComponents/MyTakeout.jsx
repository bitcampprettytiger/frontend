...
import {
  ...
  fetchPaymentList,
  API_BASE_URL,
} from '../../Home/HomeComponents/HomeApi.jsx';
    ...
    const fetchOrderAndPaymentData = async () => {
      try {
        ...
        const todayOrders = orderData.filter(
          (order) => order.orderDate && order.orderDate.split('T')[0] === today
        );
        console.log("Today's orders:", todayOrders); // 오늘의 주문 로깅

        const uniqueStores = [
          ...new Set(todayOrders.map((order) => order.vendorName)),
        ];
        ...
        orderData.forEach((order) => {
          const key = order.orderDate
            ? `${order.vendor.vendorName}-${
              ...
              }-${new Date(order.orderDate).getHours()}:${new Date(
                order.orderDate
              ).getMinutes()}`
            : '';
      ...
      <div className="mytakeout-container">
        <div className="order-summary">
          <p>
            ...
            <span className="boldNumber">{todayStoreCount}</span>개입니다.
          </p>
          <p>
            결제 내역 : 총{' '}
            <span className="boldNumber">{orderDetail.length}</span> 건
          </p>
        </div>
        ...
        <ul className="mytakeout-list">
          {orders && orders.length > 0 ? (
            orders.map((order, index) => {
              // 첫 번째 메뉴 이름을 구합니다.
              const firstMenuName =
                order.orderedMenuDTOList &&
                order.orderedMenuDTOList[0] &&
                order.orderedMenuDTOList[0].menu &&
                order.orderedMenuDTOList[0].menu.menuName;
              // 전체 메뉴 개수를 구합니다.
              const totalMenus = order.orderedMenuDTOList
                ? order.orderedMenuDTOList.length
                : 0;
              ...
              return (
                <li className="mytakeout-item" key={order.id || index}>
                  <div className="mytakeout-date">
                    ...
                    <Link
                      to={`/mytakeoutdetail/order/${order.orderId}`}
                  ...
                  <div className="mytakeout-store">
                    <img
                      src={order.vendorImageDtos[0].url || '/images/roopy.png'}
                      alt={order.vendor.vendorName}
                    />
                    <div className="mytakeout-store-info">
                      <p className="store-name">{order.vendor.vendorName}</p>
                      <div className="menu-detail">
                        <p className="gray">
                          <span>{firstMenuName}</span> 외{' '}
                          <span className="darker-text">{totalMenus - 1}</span>
                          개
                        </p>
                  ...
                  <div className="review-button-container">
                    {!order.hasReviewed ? (
                      <button
                        ...
                        disabled={
                          !order || !'orderId' in order || !'id' in order.vendor
                        }
                        onClick={() => {
                          if (
                            order &&
                            ...
                            navigate(
                              `/ReviewForm/${order.orderId}/${order.vendor.id}`
                            );
                          } else {
                            console.error(
                              'order, order.orderId or order.vendor.id is undefined!'
                            );
                          }
                        }}
                      >
