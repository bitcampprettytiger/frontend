...
function Search() {
    ...
    //         const results = predefinedData.filter(item =>
    //             (item.name && item.name.includes(cleanedQuery)) ||
    //             (item.menu && item.menu.includes(cleanedQuery)) ||
    //             (item.content && item.content.includes(cleanedQuery))
    //         );
    //         setSearchResults(results);
    //         setHasSearched(true);
    //     }
    // }, [query]);
    useEffect(() => {
        // 이 부분에서 서버에 요청을 보내 검색 결과를 가져옵니다.
        // 예를 들어, axios 라이브러리를 사용할 수 있습니다.
        // 백엔드 엔드포인트와 실제 로직은 프로젝트에 따라 달라집니다.
